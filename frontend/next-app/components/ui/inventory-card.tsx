...
export function InventoryCard({ item, onAction, isAdmin }: InventoryCardProps) {
    ...
    return (
        <div className="block h-full">
            <motion.div
                whileHover={{ y: -5 }}
                className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-xl transition-all duration-300 group flex flex-col cursor-default h-full"
            >
                {/* Image Section */}
                ...
                <div className="p-5 flex-1 flex flex-col">
                    ...
                    <Link href={`/inventory/${item.id}`}>
                        <h3 className="text-lg font-bold text-slate-800 mb-1 leading-tight line-clamp-1 group-hover:text-blue-600 transition-colors cursor-pointer">
                            {item.name}
                        </h3>
                    </Link>
