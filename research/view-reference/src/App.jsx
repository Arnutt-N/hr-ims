...
export default function App() {
    ...
    // Update sidebar state when screen size changes drastically
    useEffect(() => {
    ...
    const handleInventoryMenuClick = () => {
        if (isSidebarOpen) {
            ...
            setActiveTab('inventory-all');
            // Check if on mobile, clicking menu should open sidebar if closed
    ...
    if (!currentUser) {
        return (
            <div className="flex h-screen bg-[#0f172a] font-['Noto_Sans_Thai'] relative overflow-hidden items-center justify-center p-4">
                ...
                <div className="relative z-10 bg-white/5 backdrop-blur-xl border border-white/10 p-6 md:p-10 rounded-3xl shadow-2xl max-w-lg w-full text-center animate-fade-in-up">
                    <div className="mx-auto mb-8 bg-gradient-to-br from-blue-500 to-indigo-600 p-4 rounded-2xl w-20 h-20 md:w-24 md:h-24 flex items-center justify-center shadow-lg shadow-blue-500/30 ring-4 ring-white/5">
                        <Package size={isMobile ? 32 : 48} className="text-white" />
                    </div>
                    ...
                    <div className="space-y-4">
                        <button
                            ...
                            className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold py-3 md:py-4 rounded-xl transition-all shadow-lg shadow-indigo-600/30 flex items-center justify-center gap-3 group border border-indigo-500/50"
                        >
                        ...
                        <button
                            ...
                            className="w-full bg-white/10 hover:bg-white/20 text-white font-bold py-3 md:py-4 rounded-xl transition-all border border-white/10 flex items-center justify-center gap-3 group"
                        >
    ...
    return (
        ...
        /* Mobile adjustments for printing */
        @media print {
            aside, header { display: none !important; }
            main { margin: 0; padding: 0; width: 100%; height: 100%; overflow: visible; }
        }
      `}</style>
            ...
            <aside
                className={`
                    ...
                    bg-[#0f172a] text-white shadow-2xl transition-all duration-300 ease-in-out flex flex-col border-r border-white/5 overflow-hidden
                    ${isSidebarOpen ? 'w-[80%] max-w-[300px] md:w-72 translate-x-0' : '-translate-x-full md:translate-x-0 md:w-24'}
                `}
            >
                <div className="absolute inset-0 bg-gradient-to-b from-slate-900 via-[#1e1b4b] to-[#172554] opacity-100 z-0 pointer-events-none"></div>
                <div className="absolute top-0 left-0 w-full h-full bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-5 z-0 pointer-events-none mix-blend-overlay"></div>

                <div className="h-20 md:h-24 flex items-center justify-between md:justify-center relative z-10 border-b border-white/10 px-4 md:px-0">
                    <div className="flex items-center space-x-3">
                        <div className="bg-gradient-to-br from-blue-500 to-indigo-600 p-2 md:p-2.5 rounded-xl shadow-lg shadow-blue-500/30 shrink-0">
                            <Package size={20} className="text-white md:w-6 md:h-6" />
                        </div>
                        {(isSidebarOpen || isMobile) && (
                            <div className="flex flex-col animate-fade-in">
                                <span className="font-bold text-lg md:text-xl tracking-wide text-white">IMS.Pro</span>
                                <span className="text-[9px] md:text-[10px] text-blue-300 uppercase tracking-widest font-semibold">Inventory System</span>
                            </div>
                        )}
                    </div>
                ...
                <nav className="flex-1 py-4 md:py-6 px-3 md:px-4 space-y-2 relative z-10 overflow-y-auto sidebar-scrollbar">
                    <SidebarItem icon={<LayoutDashboard />} label="Dashboard" active={activeTab === 'dashboard'} onClick={() => setActiveTab('dashboard')} isOpen={isSidebarOpen || isMobile} />

                    <div className={`transition-all duration-300 ${(!isSidebarOpen && !isMobile) ? 'flex flex-col items-center' : ''}`}>
                        <SidebarItem
                            icon={<Layers />}
                            label="คลังพัสดุ (Inventory)"
                            active={(!isSidebarOpen && !isMobile) && activeTab.startsWith('inventory')}
                        ...
                        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${(isInventoryExpanded && (isSidebarOpen || isMobile)) ? 'max-h-60 opacity-100 mt-1' : 'max-h-0 opacity-0'}`}>
                            <SidebarItem icon={<Box />} label="ทั้งหมด (All Items)" active={activeTab === 'inventory-all'} onClick={() => setActiveTab('inventory-all')} isOpen={isSidebarOpen || isMobile} isSubItem={true} />
                            <SidebarItem icon={<History />} label="ยืม (Borrow)" active={activeTab === 'inventory-durable'} onClick={() => setActiveTab('inventory-durable')} isOpen={isSidebarOpen || isMobile} isSubItem={true} />
                            <SidebarItem icon={<ClipboardList />} label="เบิก (Withdraw)" active={activeTab === 'inventory-consumable'} onClick={() => setActiveTab('inventory-consumable')} isOpen={isSidebarOpen || isMobile} isSubItem={true} />
                        </div>
                    </div>
                    ...
                    {currentUser.role === 'admin' && (
                        <SidebarItem icon={<FileText />} label="อนุมัติคำขอ (Requests)" active={activeTab === 'requests'} onClick={() => setActiveTab('requests')} isOpen={isSidebarOpen || isMobile} count={requests.filter(r => r.status === 'pending').length} />
                    )}

                    {currentUser.role === 'user' && (
                        <SidebarItem icon={<Briefcase />} label="ทรัพย์สินของฉัน (Assets)" active={activeTab === 'my-assets'} onClick={() => setActiveTab('my-assets')} isOpen={isSidebarOpen || isMobile} count={myAssets.filter(a => a.status === 'warning').length} />
                    )}

                    <SidebarItem icon={<Calendar />} label="ประวัติ (History)" active={activeTab === 'history'} onClick={() => setActiveTab('history')} isOpen={isSidebarOpen || isMobile} />
                    <SidebarItem icon={<Wrench />} label="แจ้งซ่อม (Maintenance)" active={activeTab === 'maintenance'} onClick={() => setActiveTab('maintenance')} isOpen={isSidebarOpen || isMobile} count={inventory.filter(i => i.status === 'maintenance').length} />

                    {currentUser.role === 'admin' && (
                        <SidebarItem icon={<FileBarChart />} label="รายงาน (Reports)" active={activeTab === 'reports'} onClick={() => setActiveTab('reports')} isOpen={isSidebarOpen || isMobile} />
                    )}

                    {currentUser.role === 'admin' && (
                        <SidebarItem icon={<Printer />} label="Asset Tags" active={activeTab === 'tags'} onClick={() => setActiveTab('tags')} isOpen={isSidebarOpen || isMobile} />
                    )}

                    <SidebarItem icon={<QrCode />} label="Scan Device" active={activeTab === 'scan'} onClick={() => setActiveTab('scan')} isOpen={isSidebarOpen || isMobile} />
                    <SidebarItem icon={<ShoppingCart />} label="Cart" active={activeTab === 'cart'} onClick={() => setActiveTab('cart')} isOpen={isSidebarOpen || isMobile} count={cart.length} />

                    {currentUser.role === 'admin' && (
                        <div className="pt-6 mt-6 border-t border-white/10">
                            <div className={`px-4 mb-2 text-xs font-bold text-slate-500 uppercase tracking-wider ${(!isSidebarOpen && !isMobile) && 'hidden'}`}>Admin Tools</div>
                            <SidebarItem icon={<Users />} label="จัดการผู้ใช้ (Users)" active={activeTab === 'users'} onClick={() => setActiveTab('users')} isOpen={isSidebarOpen || isMobile} />
                            <SidebarItem icon={<Settings />} label="Settings" active={activeTab === 'settings'} onClick={() => setActiveTab('settings')} isOpen={isSidebarOpen || isMobile} />
                        </div>
                    )}
                </nav>
            ...
            {/* Main Content */}
            <main className="flex-1 flex flex-col overflow-hidden relative bg-slate-50/50 w-full">
                <header className="h-16 md:h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/60 flex items-center justify-between px-4 md:px-8 z-20 sticky top-0">
                    <div className="flex items-center">
                        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 p-2 rounded-lg transition-colors mr-2 md:mr-4">
                            <Menu size={20} />
                        </button>
                        <h1 className="text-base md:text-xl font-bold text-slate-800 truncate max-w-[150px] md:max-w-none">
                            {activeTab === 'dashboard' && 'Dashboard'}
                            {(activeTab.startsWith('inventory')) && 'Inventory'}
                            {activeTab === 'requests' && 'Requests'}
                            {activeTab === 'history' && 'History'}
                            {activeTab === 'maintenance' && 'Maintenance'}
                            {activeTab === 'my-assets' && 'My Assets'}
                            {activeTab === 'tags' && 'Tags'}
                    ...
                    <div className="flex items-center space-x-2 md:space-x-6">
                        ...
                        <div className="relative">
                            ...
                            {/* Dropdown */}
                            {showNotifications && (
                                <div className="absolute right-0 mt-2 w-72 md:w-80 bg-white rounded-xl shadow-xl border border-slate-100 py-2 z-50 animate-fade-in-up">
                        ...
                        <div className="flex items-center space-x-3 pl-2 border-l border-slate-200">
                            ...
                            <div className="p-0.5 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500">
                                <img src={currentUser.avatar} alt="Profile" className="w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-white" />
                            </div>
                        </div>
                    </div>
                </header>
                ...
                <div className="flex-1 overflow-auto p-4 md:p-8 relative scroll-smooth main-scrollbar">
                    {renderContent()}
                </div>
            </main>
        </div>
...
const SidebarItem = ({ icon, label, active, onClick, isOpen, count, isSubItem, hasSubMenu, isExpanded }) => (
    <button
        onClick={onClick}
        className={`flex items-center w-full p-3 md:p-3.5 rounded-xl transition-all duration-300 group relative my-1
      ${active
    ...
    >
        ...
        {isOpen && (
            <div className="flex-1 flex justify-between items-center ml-4 overflow-hidden">
                <span className={`font-medium tracking-wide whitespace-nowrap text-sm md:text-base ${isSubItem ? 'font-normal text-slate-300' : ''}`}>{label}</span>
                {hasSubMenu ? (
    ...
    return (
        <div className="space-y-6 animate-fade-in-up max-w-4xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-xl md:text-2xl font-bold text-slate-800">System Configuration</h2>
                    <p className="text-sm text-slate-500">Manage global settings for the Inventory System.</p>
                </div>
                <button onClick={() => alert('Settings Saved!')} className="w-full md:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-lg flex items-center justify-center gap-2 shadow-md">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 space-y-8">
                <div className="space-y-4">
                    <h3 className="font-bold text-base md:text-lg text-slate-700 border-b border-slate-100 pb-2">General Info</h3>
                ...
                <div className="space-y-4">
                    <h3 className="font-bold text-base md:text-lg text-slate-700 border-b border-slate-100 pb-2">Borrowing Rules</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                ...
                <div className="space-y-4">
                    <h3 className="font-bold text-base md:text-lg text-slate-700 border-b border-slate-100 pb-2">Notifications</h3>
                    <div className="flex items-center gap-3">
                        ...
                        <label htmlFor="maintAlert" className="text-slate-600 text-sm md:text-base">Enable Email Alerts for Maintenance</label>
                    </div>
                    <div className="flex items-center gap-3">
                        <input type="checkbox" id="lineAlert" className="w-5 h-5 text-indigo-600 rounded" defaultChecked={true} />
                        <label htmlFor="lineAlert" className="text-slate-600 text-sm md:text-base">Enable LINE Notify Integration</label>
                    </div>
                </div>
            </div>
        </div>
...
const InventoryView = ({ inventory = [], addToCart, searchQuery, setSearchQuery, currentUser, filterType = 'all', title = 'Inventory Items', onRestock }) => {
    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-fade-in-up max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                    {filtered.map(item => (
                        <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-xl transition-all duration-300 group flex flex-col">
                            <div className="h-36 md:h-44 bg-slate-50/50 flex items-center justify-center text-5xl md:text-6xl relative">
                                {item.image}
                                <div className="absolute top-3 right-3">
                                    <span className={`px-2 py-1 rounded text-[9px] md:text-[10px] font-bold uppercase tracking-wide border
                  ...
                  `}>
                            ...
                            <div className="p-4 md:p-5 flex-1 flex flex-col">
                                ...
                                <h3 className="text-base md:text-lg font-bold text-slate-800 mb-1 leading-tight">{item.name}</h3>
    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-fade-in-up max-w-7xl mx-auto">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-xl md:text-2xl font-bold text-slate-800">My Active Assets</h2>
                    <p className="text-xs md:text-sm text-slate-500 mt-1">Items currently borrowed by you. Please verify status every 7 days.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
                {myAssets.map((asset) => {
                    ...
                    return (
                        <div key={asset.id} className={`bg-white p-4 md:p-6 rounded-2xl shadow-sm border ${isWarning ? 'border-red-200 bg-red-50/30' : 'border-slate-100'} flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4`}>
                            <div className="flex items-center gap-4 w-full lg:w-auto">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${isWarning ? 'bg-red-100' : 'bg-slate-100'}`}>
                                    📦
                                </div>
                                <div>
                                    <h3 className="font-bold text-slate-800 text-base md:text-lg">{asset.name}</h3>
                            ...
                            <div className="flex flex-wrap items-center gap-2 md:gap-3 w-full lg:w-auto justify-end">
                                {isWarning ? (
                                    <button
                                        ...
                                        className="flex-1 lg:flex-none bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-bold shadow-lg shadow-red-200 transition-all flex items-center justify-center gap-2 text-sm"
                                    >
                                ...
                                {asset.status === 'returning' ? (
                                    ...
                                    <>
                                        <button
                                            ...
                                            className="flex-1 lg:flex-none text-red-500 hover:bg-red-50 px-3 py-2 rounded-lg font-bold transition-all flex items-center justify-center gap-2 text-sm border border-transparent hover:border-red-100"
                                        ...
                                        <button
                                            ...
                                            className="flex-1 lg:flex-none bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 px-4 py-2 rounded-lg font-bold transition-all flex items-center justify-center gap-2 text-sm"
                                        >
...
const UserManagementView = ({ users = [], setUsers }) => {
    ...
    return (
        <div className="space-y-6 animate-fade-in-up max-w-7xl mx-auto">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-xl md:text-2xl font-bold text-slate-800">User Management</h2>
                    <p className="text-xs md:text-sm text-slate-500">Manage system access and roles.</p>
                </div>
                <button className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-2 md:px-4 rounded-lg flex items-center gap-2 shadow-md text-sm md:text-base">
                    <Plus size={18} /> <span className="hidden md:inline">Add New User</span><span className="md:hidden">Add</span>
                </button>
            </div>
                ...
                <div className="overflow-x-auto main-scrollbar">
                    <table className="w-full text-sm text-left text-slate-600">
                        <thead className="bg-slate-50 text-slate-500 uppercase text-xs">
                            <tr>
                                <th className="px-4 md:px-6 py-4">User Info</th>
                                <th className="px-4 md:px-6 py-4">Role</th>
                                <th className="px-4 md:px-6 py-4 hidden md:table-cell">Department</th>
                            ...
                            {users.map(user => (
                                <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                                    <td className="px-4 md:px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <img src={user.avatar} alt="" className="w-8 h-8 md:w-10 md:h-10 rounded-full border border-slate-200" />
                                            <div>
                                    ...
                                    <td className="px-4 md:px-6 py-4">
                                        <span className={`px-2 py-1 rounded text-[10px] md:text-xs font-bold uppercase border ${user.role === 'admin' ? 'bg-purple-50 text-purple-700 border-purple-100' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>
                                            {user.role}
                                        </span>
                                    </td>
                                    <td className="px-4 md:px-6 py-4 hidden md:table-cell">{user.department}</td>
                                    <td className="px-4 md:px-6 py-4">
        ...
        <div className="space-y-6 animate-fade-in-up max-w-7xl mx-auto">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-xs md:text-sm text-slate-500">Approve or reject inventory requests from users.</p>
                </div>
            </div>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {requests.map(req => (
                                <tr key={req.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4 font-bold whitespace-nowrap">{req.user}</td>
...
const HistoryView = ({ history = [] }) => {
    return (
        <div className="space-y-6 animate-fade-in-up max-w-7xl mx-auto">
            <h2 className="text-xl md:text-2xl font-bold text-slate-800">Transaction History</h2>
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div className="overflow-x-auto main-scrollbar">
                    <table className="w-full text-sm text-left text-slate-600">
                        <thead className="bg-slate-50 text-slate-500 uppercase text-xs">
                            <tr>
                                ...
                                <th className="px-6 py-4 min-w-[150px]">Item</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {history.map(log => (
                                <tr key={log.id} className="hover:bg-slate-50">
                                    <td className="px-6 py-4 font-mono text-slate-400 whitespace-nowrap">{log.date}</td>
                                    <td className="px-6 py-4 font-bold whitespace-nowrap">{log.user}</td>
                                    <td className="px-6 py-4 whitespace-nowrap">{log.action}</td>
                                    <td className="px-6 py-4">{log.item}</td>
                                    <td className="px-6 py-4 whitespace-nowrap"><span className="px-2 py-1 bg-slate-100 rounded text-xs">{log.status}</span></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
const MaintenanceView = ({ inventory = [] }) => {
    ...
    return (
        <div className="space-y-6 animate-fade-in-up max-w-7xl mx-auto">
            <h2 className="text-xl md:text-2xl font-bold text-slate-800">Maintenance & Repairs</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {maintenanceItems.length > 0 ? maintenanceItems.map(item => (
                    ...
                    <div className="col-span-full text-center p-12 text-slate-400">No items currently in maintenance.</div>
                )}
            </div>
        </div>
    );
    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-fade-in-up max-w-7xl mx-auto">
            <div>
                ...
                <p className="text-xs md:text-sm text-slate-500 mt-1">Welcome back, {currentUser.name}</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                {stats.map((stat, idx) => (
                    <div key={idx} className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
                        <div>
                            <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">{stat.title}</p>
                            <h3 className="text-3xl md:text-4xl font-extrabold text-slate-800">{stat.value}</h3>
                        </div>
                        <div className={`p-3 md:p-4 rounded-2xl ${stat.bg} ${stat.color} shadow-inner`}>
            ...
            {currentUser.role === 'admin' && (
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 md:p-6">
                    <h3 className="text-base md:text-lg font-bold text-slate-800 mb-4">Pending Asset Verification (Over 7 Days)</h3>
                    <div className="overflow-x-auto main-scrollbar">
                        <table className="w-full text-sm text-left text-slate-600">
                            <thead className="bg-slate-50 text-slate-500 uppercase text-xs">
                                <tr>
                                    <th className="px-4 py-3 rounded-l-lg whitespace-nowrap">User</th>
                                    <th className="px-4 py-3 whitespace-nowrap">Asset Name</th>
                                    <th className="px-4 py-3 whitespace-nowrap">Last Checked</th>
                                    <th className="px-4 py-3 rounded-r-lg whitespace-nowrap">Status</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr className="border-b border-slate-50">
                                    <td className="px-4 py-3 font-bold whitespace-nowrap">Somchai Staff</td>
                                    <td className="px-4 py-3 whitespace-nowrap">MacBook Pro M2 (Old)</td>
                                    <td className="px-4 py-3 text-red-500 whitespace-nowrap">8 days ago</td>
                                    <td className="px-4 py-3 whitespace-nowrap"><span className="bg-red-100 text-red-700 px-2 py-1 rounded text-xs font-bold">Overdue</span></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
            ...
            {currentUser.role === 'user' && myAssets.some(a => a.status === 'warning') && (
                <div className="bg-red-50 border border-red-100 rounded-2xl p-4 md:p-6 flex items-start gap-4">
                    <div className="bg-red-100 p-2 rounded-full text-red-600 shrink-0"><AlertCircle /></div>
                    <div>
                        <h3 className="font-bold text-red-800 text-base md:text-lg">Action Required: Asset Verification</h3>
                        <p className="text-red-600 text-xs md:text-sm mt-1">You have items that haven't been verified for over 7 days. Please go to "My Assets" to confirm you still possess them.</p>
...
const CartView = ({ cart, removeFromCart, setCart }) => {
    ...
    return (
        <div className="max-w-5xl mx-auto animate-fade-in-up">
            <h2 className="text-xl md:text-2xl font-bold text-slate-800 mb-6">Request Cart Summary</h2>

            {cart.length === 0 ? (
                <div className="p-12 md:p-20 text-center bg-white rounded-3xl border border-slate-100">
                ...
                <div className="space-y-6 md:space-y-8">
                    {consumables.length > 0 && (
                        <div className="bg-white rounded-2xl border border-blue-100 overflow-hidden">
                            <div className="bg-blue-50 p-4 border-b border-blue-100 flex items-center gap-2">
                                ...
                                <h3 className="font-bold text-blue-800 text-sm md:text-base">รายการเบิกวัสดุ (Withdrawal)</h3>
                            </div>
                    ...
                    {durables.length > 0 && (
                        <div className="bg-white rounded-2xl border border-purple-100 overflow-hidden">
                            <div className="bg-purple-50 p-4 border-b border-purple-100 flex items-center gap-2">
                                ...
                                <h3 className="font-bold text-purple-800 text-sm md:text-base">รายการยืมครุภัณฑ์ (Borrow)</h3>
                            </div>
                            {durables.map((item, idx) => (
                                <div key={idx} className="p-4 flex justify-between items-center border-b border-slate-50 last:border-0">
                                    <div className="flex items-center gap-3 md:gap-4">
                                        ...
                                        <div>
                                            <div className="font-bold text-slate-700 text-sm md:text-base">{item.name}</div>
                                            <div className="text-xs text-slate-400">SN: {item.serial}</div>
                    ...
                    <button onClick={() => { alert('Request Sent!'); setCart([]); }} className="w-full py-3 md:py-4 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 text-sm md:text-base">
...
const ScannerView = ({ onScan }) => {
    ...
    return (
        <div className="h-full flex flex-col items-center justify-center animate-fade-in-up p-4">
            <div className="bg-white p-6 md:p-10 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 text-center max-w-lg w-full relative overflow-hidden">
                ...
                <div className="w-20 h-20 md:w-24 md:h-24 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-6 md:mb-8 text-indigo-600 ring-8 ring-indigo-50/50">
                    <ScanLine size={36} md:size={48} />
                </div>
                <h2 className="text-2xl md:text-3xl font-bold text-slate-800 mb-2">Device Scanner</h2>
                <p className="text-sm md:text-base text-slate-500 mb-8 md:mb-10">Use your handheld scanner or manually enter the SN.</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="relative">
                        <input
                            ref={inputRef}
                            type="text"
                            className="w-full pl-5 pr-12 py-3 md:py-4 text-lg md:text-xl border-2 border-slate-200 rounded-2xl focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all font-mono text-center text-slate-700 placeholder-slate-300"
                    ...
                    <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 md:py-4 rounded-2xl transition-all shadow-lg shadow-indigo-600/30 text-base md:text-lg">
                        Process Scan
                    </button>
                </form>

                <div className="mt-8 md:mt-10 pt-6 border-t border-slate-100 text-xs text-slate-400 flex justify-between">
...
const TagGeneratorView = ({ inventory = [] }) => {
    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-fade-in-up max-w-7xl mx-auto h-full flex flex-col">
            <div>
                <h2 className="text-xl md:text-2xl font-bold text-slate-800">Asset Tag Generator</h2>
                <p className="text-xs md:text-sm text-slate-500 mt-1">Generate and print QR codes/Barcodes for your inventory.</p>
            </div>

            <div className="flex flex-col lg:flex-row gap-6 md:gap-8 flex-1 min-h-0">
                {/* List */}
                <div className="lg:flex-1 bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-[400px] lg:h-auto">
                    ...
                    <div className="overflow-y-auto flex-1 p-2 main-scrollbar">
                        {inventory.map(item => (
                            <div
                                key={item.id}
                    ...
                    ${selectedItem?.id === item.id
                            ...
                            >
                                <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-lg flex items-center justify-center text-xl md:text-2xl border border-slate-100">
                                    {item.image}
                                </div>
                                <div>
                ...
                <div className="lg:w-96 bg-white rounded-2xl shadow-xl border border-slate-100 flex flex-col">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[radial-gradient(#e2e8f0_1px,transparent_1px)] [background-size:16px_16px] min-h-[300px] lg:min-h-0">
                        {selectedItem ? (
                            <div className="bg-white p-6 rounded-lg shadow-lg border border-slate-200 text-center w-full max-w-[280px]">
                                <div className="mb-4">
                                    ...
                                    <img
                                        ...
                                        alt="QR Code"
                                        className="mx-auto w-32 h-32 md:w-40 md:h-40 mix-blend-multiply"
                                    />
                                </div>
                                <div className="font-bold text-slate-800 text-base md:text-lg mb-1">{selectedItem.name}</div>
                    ...
                    <div className="p-6 border-t border-slate-100 bg-slate-50">
                        <button
                            ...
                            onClick={handlePrint}
                            className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white py-3 rounded-xl font-bold shadow-lg shadow-indigo-200 transition-all flex items-center justify-center gap-2 text-sm md:text-base"
                        >
                            <Printer size={18} /> Print Label
                        </button>
                        <button
                            disabled={!selectedItem}
                            className="w-full mt-3 bg-white hover:bg-slate-50 disabled:opacity-50 text-slate-600 border border-slate-200 py-3 rounded-xl font-bold transition-all flex items-center justify-center gap-2 text-sm md:text-base"
                        >
...
const ReportsView = () => {
    return (
        <div className="space-y-6 md:space-y-8 animate-fade-in-up max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-xl md:text-2xl font-bold text-slate-800">Reports & Analytics</h2>
                    <p className="text-xs md:text-sm text-slate-500">System usage summary and monthly reports.</p>
                </div>
                <div className="flex gap-2 w-full md:w-auto">
                    <button className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-white border border-slate-200 px-4 py-2 rounded-lg text-slate-600 hover:bg-slate-50 transition-colors text-sm font-bold">
                        <Calendar size={16} /> Last 30 Days
                    </button>
                    <button className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition-colors text-sm font-bold shadow-md">
